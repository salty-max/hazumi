/** Starter sketches, written the way a user would write them. */
...
return ({ background, circle, fill, width, height, t }) => {
  ...
  {
    name: 'Noise field',
    code: `// s is the same context, available during setup.
const points = Array.from({ length: 2500 }, () => ({
  x: s.random.range(0, s.width),
  y: s.random.range(0, s.height),
}));

return ({ background, circle, fill, noise, width, height, t }) => {
  ...
  },
  {
    ...
    code: `return ({ background, push, pop, translate, rotate, rect, fill, width, height, t }) => {
...
return ({ background, circle, fill, stroke, strokeWeight, with: scoped, width, height }) => {
  ...
  {
    name: 'Text',
    code: `return ({ background, text, textSize, textAlign, textFont, fill, width, height, t }) => {
