...
describe('hasExplicitImport', () => {
  test('detects an existing import', () => {
    expect(hasExplicitImport("import { sketch } from 'matter';")).toBe(true);
...
describe('matterAutoImport', () => {
  test('only transforms matching files', () => {
    ...
    expect(plugin.transform('circle();', '/a/b.sketch.ts')).not.toBeNull();
    expect(plugin.transform('circle();', '/a/b.ts')).toBeNull();
  });

  test('returns null when nothing changed, so the bundler can skip it', () => {
    expect(matterAutoImport().transform('const x = 1;', '/a/b.sketch.ts')).toBeNull();
  });
  ...
  test('the include pattern is configurable', () => {
    ...
    expect(plugin.transform('circle();', '/x.sketch.ts')).toBeNull();
  });

  test('produces code the bundler can consume', () => {
    const result = matterAutoImport().transform('circle(1,2,3);', '/s.sketch.ts');
 ...
 * the SketchContext declaration in the shipped types.
 */
describe('drift against the real context', () => {
  ...
  async function declaredContextMembers(): Promise<string[]> {
    ...
    const start = source.indexOf('interface SketchContext {');
  ...
  test('every context member is in the auto-import list', async () => {
    ...
    const missing = declared.filter(
      (name) =>
        !CONTEXT_MEMBERS.includes(name) && !NON_IMPORTABLE_MEMBERS.includes(name),
    );
