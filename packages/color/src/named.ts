...
/** Hex digits for a CSS colour name, or undefined if it is not one. */
export function namedColorHex(name: string): string | undefined {
  if (table === null) {
    table = new Map();
    for (const pair of PACKED.split(',')) {
      const [key, hex] = pair.split(' ');
      if (key !== undefined && hex !== undefined) table.set(key, hex);
    }
  }
  return table.get(name);
}

/** `transparent` is a keyword rather than a colour, so it is handled apart. */
