...
export const HELP = `create-matter — scaffold a Vite + Matter project
...
Examples:
  bun create matter
  bun create matter my-game --template game
  bun packages/create-matter/src/index.ts demo --local --yes
`;
...
export function parseArgs(argv: readonly string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--") {
      throw new UserError("Unexpected --; create-matter takes at most one directory.");
    }
    ...
    if (directory !== undefined) {
      throw new UserError("create-matter takes at most one directory.");
    }
    directory = arg;
  }
