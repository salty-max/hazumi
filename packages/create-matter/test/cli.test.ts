...
describe("run", () => {
  ...
  test("--help prints usage and writes nothing", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "create-matter-cli-"));
    const io = captureIo();
    ...
    expect(code).toBe(0);
    expect(io.stdoutText()).toContain("scaffold a Vite + Matter project");
  ...
  test("--local rewrites matter to a file: URL", async () => {
    const cwd = mkdtempSync(join(tmpdir(), "create-matter-cli-"));
    const io = captureIo();
    ...
    expect(pkg.dependencies.matter.startsWith("file://")).toBe(true);
    expect(pkg.dependencies.matter.endsWith("/packages/matter")).toBe(true);
    const localPkg = JSON.parse(readFileSync(join(cwd, "demo", "package.json"), "utf8")) as {
      overrides: Record<string, string>;
    };
    expect(localPkg.overrides["@matter/math"]?.endsWith("/packages/math")).toBe(true);
