...
function packageJson(options: ProjectFilesOptions): string {
  ...
  if (options.autoImport) {
    devDependencies["@hazumi/vite-plugin"] = options.vitePluginSpec;
  }
  ...
  return `${JSON.stringify(
    {
      ...
      include: autoImport ? ["src", "node_modules/@hazumi/vite-plugin/globals.d.ts"] : ["src"],
    },
    null,
    2,
  )}\n`;
...
function viteConfig(autoImport: boolean): string {
  if (autoImport) {
    return `import { hazumiAutoImport } from "@hazumi/vite-plugin";
...
function sceneImports(template: TemplateKind, autoImport: boolean): string {
  ...
  if (template === TemplateKind.Game) {
    lines.push(`import { collision } from "hazumi/math";`);
  }
  if (!autoImport) {
